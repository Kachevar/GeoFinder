...
with open('map.html', 'r+', encoding='utf-8') as file:
    ...
    <div id="statistics" style="width: 100%; background: white; padding: 30px; box-sizing: border-box; border-top: 2px solid #ddd; margin-top: 20px;">
        ...
        <div style="display: flex; justify-content: center; gap: 30px; margin-bottom: 25px;">
            <div style="text-align: center;">
                <div style="font-size: 28px; font-weight: bold; color: #2c3e50;">{len(schools)}</div>
            ...
            <div style="text-align: center;">
                <div style="font-size: 28px; font-weight: bold; color: #e74c3c;">{len(far_schools)}</div>
    ...
    stats_html += """
            ...
            <div style="flex: 1; max-width: 500px;">
                <h4 style="color: #e74c3c; border-bottom: 2px solid #e74c3c; padding-bottom: 8px;">
                    ...
                    Школы вне радиуса 3 км от УДО ({len(far_schools)})
                </h4>
                <div style="max-height: 400px; overflow-y: auto; padding: 10px;">
    """
    ...
    for school_id in far_schools:
        lat, lon = school_id.split('_')
        school_info = schools[(schools['lat'] == float(lat)) & (schools['lon'] == float(lon))].iloc[0]
        stats_html += f"<p style='margin: 8px 0; padding: 5px; background: #fff8f8; border-left: 3px solid #e74c3c;'><i class='fa fa-school' style='color: #e74c3c; margin-right: 10px;'></i>{school_info['name']}</p>"
    ...
    # Вставляем легенду в тело карты
    content = content.replace('</body>', legend_html + '</body>')
    
    # Находим конец карты и вставляем статистику после нее
    if '</body>' in content:
